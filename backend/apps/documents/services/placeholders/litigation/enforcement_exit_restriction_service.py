...
class EnforcementExitRestrictionRequestService(BasePlaceholderService):
    """生成强制执行申请书中的“限制出境申请事项”占位符."""

    name: str = "enforcement_exit_restriction_request_service"
    display_name: str = "诉讼文书-强制执行申请书限制出境申请事项"
    description: str = "根据被申请人信息生成限制出境申请事项"
    category: str = "litigation"
    placeholder_keys: ClassVar = [LitigationPlaceholderKeys.ENFORCEMENT_EXIT_RESTRICTION_REQUEST]
    ...
    def generate(self, context_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not target_text:
            logger.warning("未找到被申请人信息，限制出境申请事项为空: case_id=%s", case_id)
            return {LitigationPlaceholderKeys.ENFORCEMENT_EXIT_RESTRICTION_REQUEST: ""}

        text = f"请求贵院依法决定对被申请人{target_text}采取限制出境的措施。"
    ...
    def _build_target_text(self, *, case_id: int) -> str:
        ...
        for party in respondents:
            ...
            if legal_representative:
                segment = f"{company_name}的法定代表人{legal_representative}"
