...
class FeeNoticeCheckService:
    ...
    def check_fee_notices(
        ...
        # 获取案件比对信息
        case_info = self.comparison_service.get_case_info_for_comparison(cast(int, sms.case.id))  # type: ignore[redundant-cast]
        ...
        for file_path in fee_notice_files:
            check_item = self._check_single_file(file_path, cast(int, sms.case.id), case_info)  # type: ignore[redundant-cast]
            if check_item:
