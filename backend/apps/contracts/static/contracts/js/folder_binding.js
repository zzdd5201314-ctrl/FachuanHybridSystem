/**
 * @deprecated 此文件已废弃，请使用 folder_binding_app.js (Alpine.js 通用组件) 替代
 * 新组件位置: backend/apps/cases/static/cases/js/folder_binding_app.js
 * 迁移日期: 2026-01-08
 * 保留此文件仅供参考和回滚使用
 *
 * 合同文件夹绑定功能 JavaScript
 *
 * 功能包括：
 * - 绑定状态显示和更新
 * - 绑定对话框显示逻辑
...
(function() {
    ...
    var FolderBindingManager = {
        ...
        getContractId: function() {
            var match = window.location.pathname.match(/\/admin\/contracts\/contract\/(\d+)\/change\//);
        ...
        loadBindingStatus: function() {
            ...
            fetch('/api/v1/contracts/' + contractId + '/folder-binding')
        ...
        createOrUpdateBinding: function(folderPath) {
            ...
            if (!contractId) {
                this.showPathError('无法获取合同ID');
                return;
            }

            var url = '/api/v1/contracts/' + contractId + '/folder-binding';
