...
- 法院短信 Admin 详情页“关联文书”支持手动重命名：用户仅可修改文件名主体，扩展名保持锁定不变；重命名后下载文件名与文书引用路径同步更新。
- 新增“关联文书”批量下载能力：在详情页提供“批量下载全部文书”按钮，支持将当前短信关联文书打包为 ZIP 下载（`courtsms_<sms_id>_documents.zip`）。
- 修复 `CourtSMS` 详情页渲染异常：处理 Django 6 下 `format_html` 无参数调用导致的 `TypeError`，恢复 `change_view` 正常访问。
