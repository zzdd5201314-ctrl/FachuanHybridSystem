...
- **一张网担保申请自动填写增强（automation/scraper/court_zxfw_guarantee）**：
  - 法院选择改为多候选名逐级匹配（全称 → 短名 → 关键词），最多8轮重试，显著提升模糊匹配成功率。
  - 保全类型、保全类别、提交人身份改用 `_click_radio_in_form_item` 精确定位表单项后点击 radio，避免页面存在多个同名 radio 时误点。
  - 新增被申请人财产线索自动填充（`_build_primary_respondent_property_clue`），从案件当事人财产线索中提取主要被申请人财产信息写入申请表单。
...
- **注册页首次自动初始化能力**
  ...
  - 首次点击后会自动创建默认超级管理员账户：`法穿 / 1234qwer`
