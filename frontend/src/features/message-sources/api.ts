...
export const messageSourceApi = {
  ...
  create: (data: {
    display_name: string; source_type: string; credential_id: number;
    is_enabled?: boolean; poll_interval_minutes?: number;
    sync_since?: string | null; imap_host?: string; imap_account?: string;
    sender_whitelist?: string; sender_blacklist?: string;
  }): Promise<MessageSource> =>
    api.post('sources', { json: data }).json(),
  update: (id: number, data: Partial<{
    display_name: string; is_enabled: boolean; poll_interval_minutes: number;
    sync_since: string | null; imap_host: string; imap_account: string;
    sender_whitelist: string; sender_blacklist: string;
  }>): Promise<MessageSource> =>
